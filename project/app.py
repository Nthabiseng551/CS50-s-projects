...
def form():
    ...
    if request.method == "POST":
        ...
        if requests != 0:
            ...
            db.execute("UPDATE requests SET user_email=?, user_number=?, age=?, username=?, WHERE username=?", request.form.get("username"))
            db.execute("UPDATE requests SET counsellor_gender=?, psychological_concerns=?, setting=?, WHERE username=?", request.form.get("username"))
