...
def index():
    ...
    stocks = db.execute("SELECT symbol, SUM(shares) as total_shares FROM transactions WHERE user_id = ? GROUP BY symbol HAVING totalshares > 0", session["user_id"])
...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, type) VALUES (?, ?, ?, ?, ?))", user_id, symbol=symbol, shares=shares, price=price, type=)
