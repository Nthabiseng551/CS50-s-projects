...
def index():
    ...
    stocks = db.execute("SELECT symbol, price, SUM(shares) as FROM transactions WHERE user_id = ? GROUP BY symbol", user_id)
    ...
    return render_template("index.html", stocks=stocks, cash=cash, total_value=total_value, grand_total=grand_total, usd=usd)
